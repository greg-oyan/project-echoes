...
| Full atomic build 1 | partially implemented | Every failed or weaker attempt remains preserved in ADR 0015 and the experiment log. Sole-worker attempt 5k validated all 26 Tier 3 checkpoint batches, completed all 600 frozen null replicates and calibration, completed global candidate ranking, and wrote four aligned candidate/evidence parts before an adjacent one-bin BM25 reproduction check stopped materialization. The complete 10.16-GiB staging tree, 968-part primary checkpoint, 52 Tier 3 checkpoint files, null/calibration outputs, candidate parts, stderr, and posthoc provenance inventory remain reusable. The exact/adjacent/>1-bin reconciliation contract and execution-attempt sidecars pass focused tests. The next sole-worker resume starts at the candidate-materialization boundary; it must not repeat retrieval, Tier 3, or null simulation |
