...
KQ_SUPPLEMENT_SCHEMA_VERSION = 1
...
def derive_supplemented_analysis_stream(
    ...
    *,
    ...
    ``qere``: byte-identical to the primary-only derived stream — the
    supplement adds nothing and removes nothing.
    ...
    if analysis_reading == "qere":
        return derive_analysis_stream(primary_tokens, analysis_reading="qere")
    ...
    return stream.cast(pl.Schema(ANALYSIS_TOKEN_POLARS_SCHEMA)).select(ANALYSIS_TOKEN_COLUMNS)
...
def validate_kq_supplement(
    ...
    *,
    ...
    qere_stream = derive_supplemented_analysis_stream(
        primary_tokens, ketiv, registry, analysis_reading="qere"
    )
    base_stream = derive_analysis_stream(primary_tokens, analysis_reading="qere")
    if not qere_stream.equals(base_stream):
    ...
    ketiv_stream = derive_supplemented_analysis_stream(
        primary_tokens, ketiv, registry, analysis_reading="ketiv"
    )
