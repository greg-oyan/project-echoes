...
The verified `final-discovery-v1` handoff snapshot, **2026-09-24 at 16:25 UTC**, confirms all 11 computational stages and passing all-stage validation: 5,962,874 evidence rows, 1,748,143 candidate pairs, 1,960 statistically eligible Tier A candidates, and 100 exploratory Tier B candidates. Finalization receipts and the selected private review bundle are authenticated; a separate B2 inventory and critical-proof recheck passed. Eligibility does not establish a discovery, and human review remains incomplete. The full 92.5-GB artifact package remains in private B2; the verified local handoff is approximately 23.9 MB. The provider independently confirmed the instance is stopped after retrieval. See the [run-status record](docs/final-discovery-run-status.md) for proof hashes and remaining review obligations.
